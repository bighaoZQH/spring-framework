...
final class PostProcessorRegistrationDelegate {
	 ...
	 * 这个是用于存放已经执行过的处理器，通过这个set集合来判断是否已经处理过了
	 *
	 * 这里面有3个list
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (BeanFactoryPostProcessor postProcessor : beanFactoryPostProcessors) {
				...
				if (postProcessor instanceof BeanDefinitionRegistryPostProcessor) {
					...
					// 优先执行手动add的BeanDefinitionRegistryPostProcessor中的
			...
			/**
			 * 这个currentRegistryProcessors放的是要去执行的实现了BeanDefinitionRegistryPostProcessor接口的对象
			 *
			 * BeanDefinitionRegistryPostProcessor的执行顺序是
			 * 先执行Spring自己内部的，然后再执行我们自定义的，
			 * 所以一开始放的是Spring内部自己实现了BeanDefinitionRegistryPostProcessor接口的对象，去执行
			 * 然后清除，再遍历放入我们自己的，再去执行，然后清除这个list
			 */
			...
			// 合并list，将spring内部的和我们自定义的合并到一起，这个list是最后用于执行BeanFactoryPostProcessor的回调
			registryProcessors.addAll(currentRegistryProcessors);
			// 执行Spring内部的BeanDefinitionRegistryPostProcessor，到此我们手动add和spring内部的都执行完了
			invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry, beanFactory.getApplicationStartup());
			// 清除list的里数据，接下来放入其他要执行的BeanDefinitionRegistryPostProcessor
			currentRegistryProcessors.clear();

			/** ====== 到此执行完成我们手动add的和Spring内部的BeanDefinitionRegistryPostProcessor === */

			// Next, invoke the BeanDefinitionRegistryPostProcessors that implement Ordered.
			// 上面spring回去解析配置类，因此可能会扫描到我们项目中配置的其他BeanDefinitionRegistryPostProcessor，因此再重新获取一遍名称
			...
			while (reiterate) {
				reiterate = false;
				// 道理同上，所以重新获取一边，因为可能有新的..
				postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
				for (String ppName : postProcessorNames) {
					// 接下来执行所有还未处理的BeanDefinitionRegistryPostProcessors
		...
		/**
		 * 下面的逻辑也是一样，因为上面执行的后置处理器可能还会向容器中添加新的后置处理器，
		 * 因此，这里又要重新获取一遍。当然不会重复执行，因为执行过的会添加到processedBeans这个set集合中，
		 * 如果已经存在了，就不执行了
		 */
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
