...
public abstract class AnnotationConfigUtils {
	...
	/**
	 * 根据作用域为Bean应用引用的代码模式
	 * https://www.cnblogs.com/pc-boke/articles/9160329.html
	 * https://blog.csdn.net/u013423085/article/details/82872533
	 *
	 * 这个是做什么用的？
	 * 因为spring bean的作用域除了单例和原型，还有session、request等等
	 * 试想，beanA是单例的，在容器启动就会创建，beanB是session的，只有新会话来访问才会被创建，
	 * 但如果beanA要注入beanB，那么此时beanB还没有被创建，因此Spring就用一个代理来解决这个问题。
	 * 当request注入到signleton bean时，也是一样。
	 *
	 * 当beanB是一个接口时，指定 ScopedProxyMode.INTERFACES，
	 * spring会通过jdk动态代理来创建实现了目标接口的代理对象。
	 *
	 * 当beanB是一个类时，指定 ScopedProxyMode.TARGET_CLASS，
	 * srping会通过CGLib来创建基于目标类的代理对象。
	 */
	static BeanDefinitionHolder applyScopedProxyMode(
		...
		// 如果配置的@Scope注解的proxyMode属性值为NO，则不应用代理模式，默认就是ScopedProxyMode.DETAULT，等同于NO
